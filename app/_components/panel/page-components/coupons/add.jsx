...
const AddCoupon = ({ open, setOpen }) => {
    ...
    const { data: tours, isPending: isToursPending, isSuccess: isToursSuccess, isLoading: isToursLoading } = useQuery({
        queryKey: ['toursSearch', userInput],
        queryFn: async () => {
            try {
    ...
    // Fetch rented cars
    const [rentedCarInput, setRentedCarInput] = useState('');
    ...
    const { data: rentedCars, isPending: isRentedCarsPending, isSuccess: isRentedCarsSuccess, isLoading: isRentedCarsLoading } = useQuery({
        queryKey: ['rented-cars-search', userInput],
        queryFn: async () => {
            try {
    ...
    // Coupon add function
    const { mutate: addCoupon, isPending: isAddCouponPending, isSuccess: isAddCouponSuccess, error: addCouponError } = useMutation({
        mutationFn: async (formData) => {
            try {
                ...
                const { data } = await axios.post(`/api/panel/coupons/`, bodyFormData)
                return data;
            } catch (error) {
                throw new Error(error.response.data.error)
            }
        ...
        <Sheet open={open} onOpenChange={setOpen} className='w-full'>
            <SheetContent className='overflow-auto'>
                <SheetHeader className='text-left'>
                    ...
                    <SheetDescription>
                        Coupons can be used to give discount to the users or for a marketing campaign.
                    </SheetDescription>
                    <Message variant={addCouponError?.message ? 'destructive' : 'default'} message={addCouponError?.message || isAddCouponSuccess && `Coupon has been added successfully.`} />
                </SheetHeader>
                <div className="mt-5">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(addCoupon)} className="space-y-3">
                            <FormField
                            ...
                            />
                            <Button type="submit" className='w-full' disabled={isAddCouponPending}>
                                {isAddCouponPending ?
