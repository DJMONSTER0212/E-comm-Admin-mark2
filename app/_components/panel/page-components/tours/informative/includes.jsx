...
import { PlusCircle, Pencil, Check, X, MoreVertical, ArrowUpDown } from 'lucide-react';
...
const TourIncludes = ({ tour }) => {
    const queryClient = useQueryClient();
    const { toast } = useToast()
    const [includesOrder, setIncludesOrder] = useState(tour.includes || [])
    // Tour include sort function
    const { mutate: sortIncludes } = useMutation({
        mutationFn: async () => {
            try {
                const { data } = await axios.patch(`/api/panel/tours/${tour._id}/includes/`, { action: 'sort', includes: includesOrder })
                return data;
            } catch (error) {
    ...
    // Update includes order state on change in tour
    useEffect(() => {
        if (tour) {
            setIncludesOrder(tour.includes)
        }
    }, [tour])
    return (
        <>
            <div className='lg:col-span-2'>
                <Card>
                    <CardHeader>
                        <CardTitle size='lg'>What{"'s"} included in the tour</CardTitle>
                        <CardDescription>Write all the items, rules, services, etc which are or are not included in <span className='text-foreground font-medium'>{tour.name}</span> tour.</CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <AddInclude open={isAddIncludeOpen} setOpen={setIsAddIncludeOpen} tour={tour} />
            {selectedInclude && <EditInclude open={isEditIncludeOpen} setOpen={setIsEditIncludeOpen} tour={tour} include={selectedInclude} />}
            {selectedInclude && <DeleteIncludeDialog open={isDeleteIncludeDialogOpen} setOpen={setIsDeleteIncludeDialogOpen} tour={tour} include={selectedInclude} />}
        </>
    )
}
