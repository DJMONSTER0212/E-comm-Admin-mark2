...
const DeleteCancellationRuleDialog = ({ open, setOpen, tour, cancellationRule, callback }) => {
    ...
    const { mutate: deleteCancellationRule, isPending: isDeleteCancellationRulePending, isSuccess: isDeleteCancellationRuleSuccess, error: deleteCancellationRuleError } = useMutation({
        mutationFn: async () => {
            try {
                const { data } = await axios.delete(`/api/panel/tours/${tour._id}/cancellation-rules/${cancellationRule._id}`)
                return data;
            } catch (error) {
