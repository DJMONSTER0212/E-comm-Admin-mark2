...
const formSchema = z.object({
    ...
    hoursBeforeStartingTime: z.coerce.number().min(1, { message: 'Hours before the tour start time is required' }),
    rule: z.string().min(1, { message: 'Rule is required' }),
});

const EditCancellationRule = ({ open, setOpen, tour, cancellationRule, side }) => {
    ...
    // Tour cancellation rule update function
    const { mutate: updateCancellationRule, isPending: isUpdateCancellationRulePending, isSuccess: isUpdateCancellationRuleSuccess, error: updateCancellationRuleError, reset: updateCancellationRuleReset } = useMutation({
        mutationFn: async (formData) => {
            try {
                const { data } = await axios.put(`/api/panel/tours/${tour._id}/cancellation-rules/${cancellationRule._id}`, formData)
                return data;
            } catch (error) {
