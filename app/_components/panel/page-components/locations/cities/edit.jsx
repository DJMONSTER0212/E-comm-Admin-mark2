...
const EditCity = ({ open, setOpen, city, side }) => {
  ...
  const selectedCountry = useWatch({ control: form.control, name: 'countryId', defaultValue: city.country?._id })
  // Fetch countries
  ...
  const { mutate: updateCity, isPending: isUpdateCityPending, isSuccess: isUpdateCitySuccess, error: updateCityError, reset: updateCityReset } = useMutation({
    mutationFn: async (formData) => {
      try {
        ...
        const { data } = await axios.put(`/api/panel/cities/${city._id}`, bodyFormData)
        return data;
      } catch (error) {
        throw new Error(error.response.data.error)
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['cities', city.stateId],
      })
    },
  })
  // Update default values on change in city values
  useEffect(() => {
    if (city) {
      const updateValues = {
        name: city.name,
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={setOpen} className='w-full'>
        <SheetContent side={side} className='overflow-auto'>
          <SheetHeader className='text-left'>
            ...
            <SheetDescription>
              Cities are used to list all the tours for a city.
            </SheetDescription>
            <Message variant={updateCityError?.message ? 'destructive' : 'default'} message={updateCityError?.message || isUpdateCitySuccess && `City has been updated successfully.`} />
          </SheetHeader>
          <div className="mt-5">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(updateCity)} className="space-y-3">
                <FormField
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <FormControl disabled={!(selectedCountry || city.country?._id) && true}>
                            <Button
                              variant="outline"
                              role="combobox"
                              className={cn(
                ...
                />
                <Button type="submit" className='w-full' disabled={isUpdateCityPending}>
                  {isUpdateCityPending ?
      ...
      <AddState countryId={city.country?._id || selectedCountry} open={isAddStateOpen} setOpen={setIsAddStateOpen} />
    </>
  )
}
