...
import DeleteVisaLeadDialog from "./delete-dialog";
import ViewVisaLead from "./view";
import { leadStatuses } from "@/app/_conf/constants/constant";
import fileDownload from "js-file-download";

const VisaLeadsTable = () => {
    const { toast } = useToast();
    const searchparam = useSearchParams();
    // Fetch visa lead
    const { data, error, isPending, isSuccess, refetch } = useQuery({
        queryKey: ['visa-leads', searchparam.toString()],
        queryFn: async () => {
            try {
                const { data } = await axios.get(`/api/panel/visa-leads?${searchparam.toString()}`)
                return data;
            } catch (error) {
    ...
    // Visa lead export
    const { isLoading: isExportedVisaLeadsLoading, refetch: refetchExportedVisaLeads } = useQuery({
        queryKey: ['export-visa-leads', searchparam.toString()],
        queryFn: async () => {
            try {
                const { data } = await axios.get(`/api/panel/visa-leads?all=true&export=true&${searchparam.toString()}`)
                fileDownload(data, 'visa-leads.csv')
                return data;
            } catch (error) {
                throw new Error(error.response.data.error)
            }
    ...
    const exportVisaLeads = () => {
        refetchExportedVisaLeads();
    };
    // Mutate visa lead
    const { mutate: mutateVisaLead } = useMutation({
        mutationFn: async ({ visaLead, status }) => {
            try {
    ...
    // Open state for delete and view visa lead
    const [isViewVisaLeadOpen, setIsViewVisaLeadOpen] = useState(false)
    const [isDeleteAlertOpen, setIsDeleteAlertOpen] = useState(false)
    const [selectedVisaLead, setSelectedVisaLead] = useState()
    const columns = [
        {
            accessorKey: "name",
            header: ({ column }) => (
            ...
            cell: ({ row }) => (
                <div className="flex flex-col">
                    ...
                    <p className="text-sm font-normal text-muted-foreground">From <span className="lowercase">{row.original.city}</span></p>
                </div>
            ),
            enableSorting: false,
            enableHiding: false,
        ...
        },
        {
            accessorKey: "country",
            header: ({ column }) => (
                <DataTableHeader column={column} title="Applied visa for" />
            ),
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex gap-2 justify-end">
                    <Button onClick={() => { setSelectedVisaLead(row.original); setIsViewVisaLeadOpen(true) }} variant='default' size='icon' className='w-8 h-8'><Pencil className='w-4 h-4' /></Button>
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-[160px]">
                            <DropdownMenuSub>
                                ...
                                <DropdownMenuSubContent>
                                    <DropdownMenuRadioGroup
                                        ...
                                        onValueChange={(value) => { mutateVisaLead({ visaLead: row.original, status: value }) }}
                                    >
                            ...
                            <DropdownMenuItem
                                ...
                                onClick={() => {
                                    setSelectedVisaLead(row.original)
        ...
        <>
            ...
            {isSuccess &&
                <DataTable
                    ...
                    tableOperations={<Button onClick={exportVisaLeads} variant='default' size='sm' className='h-8'>{isExportedVisaLeadsLoading ? <Loader2 className={`mr-2 h-4 w-4 animate-spin`} /> : <Download className={`mr-2 h-4 w-4`} />}Export listed leads</Button>}
                />
            }
            {selectedVisaLead && <DeleteVisaLeadDialog open={isDeleteAlertOpen} setOpen={setIsDeleteAlertOpen} visaLead={selectedVisaLead} />}
            {selectedVisaLead && <ViewVisaLead open={isViewVisaLeadOpen} setOpen={setIsViewVisaLeadOpen} visaLead={selectedVisaLead} />}
        </>
    );
}

export default VisaLeadsTable;