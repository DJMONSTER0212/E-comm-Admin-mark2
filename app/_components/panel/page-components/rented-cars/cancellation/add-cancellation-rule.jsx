...
const formSchema = z.object({
    ...
    hoursBeforeStartingTime: z.coerce.number().min(1, { message: 'Hours before the pickup is required' }).refine((value) => {
        const numericValue = parseFloat(value);
        return numericValue >= 0;
    }, {
        message: 'Hours before the pickup start must be greater than 0',
    }),
    rule: z.string().min(1, { message: 'Rule is required' }),
});

const AddCancellationRule = ({ open, setOpen, rentedCar, side }) => {
    ...
    // Rented car cancellation rule add function
    const { mutate: addCancellationRule, isPending: isAddCancellationRulePending, isSuccess: isAddCancellationRuleSuccess, error: addCancellationRuleError } = useMutation({
        mutationFn: async (formData) => {
            try {
                const { data } = await axios.post(`/api/panel/rented-cars/${rentedCar._id}/cancellation-rules/`, formData)
                return data;
            } catch (error) {
