...
const EditInclude = ({ open, setOpen, rentedCar, include, side }) => {
    ...
    // Rented car include item update function
    const { mutate: updateInclude, isPending: isUpdateIncludePending, isSuccess: isUpdateIncludeSuccess, error: updateIncludeError, reset: updateIncludeReset } = useMutation({
        mutationFn: async (formData) => {
            try {
                const { data } = await axios.put(`/api/panel/rented-cars/${rentedCar._id}/includes/${include._id}`, formData)
                return data;
            } catch (error) {
        ...
        <Sheet open={open} onOpenChange={setOpen} className='w-full'>
            <SheetContent side={side} className='overflow-auto'>
                <SheetHeader className='text-left'>
                    ...
                    <SheetDescription>
                        Include contains information of what is and what is not included in rented car
                    </SheetDescription>
