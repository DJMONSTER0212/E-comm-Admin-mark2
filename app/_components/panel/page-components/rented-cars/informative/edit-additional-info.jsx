...
const EditAdditionalInfo = ({ open, setOpen, rentedCar, additionalInfo, side }) => {
    ...
    // Rented car additional info update function
    const { mutate: updateAdditionalInfo, isPending: isUpdateAdditionalInfoPending, isSuccess: isUpdateAdditionalInfoSuccess, error: updateAdditionalInfoError, reset: updateAdditionalInfoReset } = useMutation({
        mutationFn: async (formData) => {
            try {
                const { data } = await axios.put(`/api/panel/rented-cars/${rentedCar._id}/additional-info/${additionalInfo.index}`, formData)
                return data;
            } catch (error) {
        ...
        <Sheet open={open} onOpenChange={setOpen} className='w-full'>
            <SheetContent side={side} className='overflow-auto'>
                <SheetHeader className='text-left'>
                    ...
                    <SheetDescription>
                        Additiona info contains some more information about rented car
                    </SheetDescription>
