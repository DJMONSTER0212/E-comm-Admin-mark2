...
const DeleteAdditionalInfoDialog = ({ open, setOpen, rentedCar, additionalInfo, callback }) => {
    ...
    const { mutate: deleteAdditionalInfo, isPending: isDeleteAdditionalInfoPending, error: deleteAdditionalInfoError } = useMutation({
        mutationFn: async () => {
            try {
                const { data } = await axios.delete(`/api/panel/rented-cars/${rentedCar._id}/additional-info/${additionalInfo.info}`)
                return data;
            } catch (error) {
