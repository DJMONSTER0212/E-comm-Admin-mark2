...
const DeleteIncludeDialog = ({ open, setOpen, rentedCar, include, callback }) => {
    const queryClient = useQueryClient()
    const router = useRouter();
    const { toast } = useToast()
    // Include item delete
    const { mutate: deleteInclude, isPending: isDeleteIncludePending, error: deleteIncludeError } = useMutation({
        mutationFn: async (include) => {
            try {
                const { data } = await axios.delete(`/api/panel/rented-cars/${rentedCar._id}/includes/${include._id}`)
                return data;
            } catch (error) {
