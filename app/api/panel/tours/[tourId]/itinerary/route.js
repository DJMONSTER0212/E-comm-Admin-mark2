...
import { itinerary } from "./getController"
import { putItinerary } from "./putController"
...
export async function GET(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await itinerary({ req, params, session })
            default:
...
export async function PUT(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await putItinerary({ req, params, session })
            default:
                return Response.json({ error: `You are not allowed to access this content.` }, { status: 400 })
        }
    }
