...
import { tourDate } from "./getController"
import { putUser } from "./putController"
import { deleteTourDate } from "./deleteController"
...
export async function GET(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await tourDate({ req, params, session })
            default:
...
export async function DELETE(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await deleteTourDate({ req, params, session })
            default:
                return Response.json({ error: `You are not allowed to access this content.` }, { status: 400 })
        }
    }
