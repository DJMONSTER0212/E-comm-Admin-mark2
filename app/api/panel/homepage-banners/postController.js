...
export const postHomepageBanner = async ({ req, params }) => {
    ...
    // Database connection
    connectDB()
    // Adding new homepage banner
    try {
        const newHomepageBanner = new homepageBannerModel(updateFields)
        await newHomepageBanner.save();
        return Response.json({ status: 200 })
    } catch (error) {
        return Response.json({ error: `Adding homepage banner failed. ${process.env.NODE_ENV == 'development' ? `Error : ${error}`: ''}` }, { status: 400 })
    }
}