...
import { tourCategories } from "./getController"
import { postTourCategory } from "./postController"
...
export async function GET(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await tourCategories({ req, params })
            default:
...
export async function POST(req) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await postTourCategory({ req })
            default:
                return Response.json({ error: `You are not allowed to access this content.` }, { status: 400 })
        }
    }
