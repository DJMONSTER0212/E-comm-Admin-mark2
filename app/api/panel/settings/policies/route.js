...
import { policiesSettings } from "./getController"
import { putPoliciesSettings } from "./putController"
...
export async function GET(req) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await policiesSettings({ req })
            default:
...
export async function PUT(req) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await putPoliciesSettings({ req })
            default:
                return Response.json({ error: `You are not allowed to access this content.` }, { status: 400 })
        }
    }
