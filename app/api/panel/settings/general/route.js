...
import { generalSettings } from "./getController"
import { putGeneralSettings } from "./putController"
...
export async function GET(req) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await generalSettings({ req })
            default:
...
export async function PUT(req) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await putGeneralSettings({ req })
            default:
                return Response.json({ error: `You are not allowed to access this content.` }, { status: 400 })
        }
    }
