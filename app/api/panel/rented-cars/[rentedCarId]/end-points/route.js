...
import { endPoints } from "./getController"
import { postEndPoint } from "./postController"
...
export async function GET(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await endPoints({ req, params, session })
            default:
...
export async function POST(req, { params }) {
    ...
    if (!session) {
        ...
        switch (session.user.role) {
            case 'sadmin':
            case 'admin':
                return await postEndPoint({ req, params, session })
            default:
                return Response.json({ error: `You are not allowed to access this content.` }, { status: 400 })
        }
    }
