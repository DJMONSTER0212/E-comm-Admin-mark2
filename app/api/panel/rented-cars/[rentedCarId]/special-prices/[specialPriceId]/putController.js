...
export const putSpecialPrice = async ({ req, params, session }) => {
    ...
    if (updateFields.rangeType == 'date') {
        ...
        const currentSpecialPrices = await rentedCarSpecialpriceModel.find({ _id: { $ne: specialPriceId }, rentedCarId, rangeType: 'date' });
    ...
    if (updateFields.rangeType == 'day') {
        ...
        const currentSpecialPrices = await rentedCarSpecialpriceModel.find({ _id: { $ne: specialPriceId }, rentedCarId, rangeType: 'day', day: updateFields.day });
    ...
    try {
        await rentedCarSpecialpriceModel.updateOne({ _id: specialPriceId }, { $set: updateFields });
        return Response.json({ status: 200 })
    } catch (error) {
        return Response.json({ error: `Updating special price failed. ${process.env.NODE_ENV == 'development' ? `Error : ${error}`: ''}` }, { status: 400 })
    }
}